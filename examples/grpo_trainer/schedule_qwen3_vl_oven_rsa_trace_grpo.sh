...
Run options:
  ...
  --conda-env <NAME>            Conda env to activate (default: verl-vllm110)
...
Resource env overrides:
  ...
  MODEL_ENABLE_GRADIENT_CHECKPOINTING, ACTOR_STRATEGY, REF_STRATEGY, ACTOR_FSDP_SIZE, ACTOR_MODEL_DTYPE,
  ...
  VERL_DISABLE_OPTIONAL_CHECKPOINT_BACKENDS defaults to nccl,nixl,hccl,kimi,mooncake.
...
LOGPROB_MICRO_BATCH_SIZE_PER_GPU="${LOGPROB_MICRO_BATCH_SIZE_PER_GPU:-}"
...
export PPO_MICRO_BATCH_SIZE_PER_GPU=$(shell_quote "$PPO_MICRO_BATCH_SIZE_PER_GPU")
export LOGPROB_MICRO_BATCH_SIZE_PER_GPU=$(shell_quote "$LOGPROB_MICRO_BATCH_SIZE_PER_GPU")
export REF_LOGPROB_MICRO_BATCH_SIZE_PER_GPU=$(shell_quote "$REF_LOGPROB_MICRO_BATCH_SIZE_PER_GPU")
...
export VERL_DISABLE_OPTIONAL_CHECKPOINT_BACKENDS=$(shell_quote "$VERL_DISABLE_OPTIONAL_CHECKPOINT_BACKENDS")
...
echo "  batch:      train=\$TRAIN_BATCH_SIZE ppo=\$PPO_MINI_BATCH_SIZE micro=\$PPO_MICRO_BATCH_SIZE_PER_GPU"
...
echo "  actor/ref:  actor=\$ACTOR_STRATEGY ref=\$REF_STRATEGY fsdp_size=\$ACTOR_FSDP_SIZE dtype=\$ACTOR_MODEL_DTYPE/\$REF_MODEL_DTYPE remove_padding=\$MODEL_USE_REMOVE_PADDING fused=\$MODEL_USE_FUSED_KERNELS grad_ckpt=\$MODEL_ENABLE_GRADIENT_CHECKPOINTING"
...
echo "  disabled ckpt backends: \$VERL_DISABLE_OPTIONAL_CHECKPOINT_BACKENDS"
